...
elseif($method_id==19):
    ...
    elseif ($method_id == 29):
        ...
        $paid_amount = $amount * $dollar_rate ;
        ...
        <form  id="form" action="'.$url.'" method="POST">
            <input value="'.$paid_amount.'" name="tAmt" type="hidden">
            <input value="'.$paid_amount.'" name="amt" type="hidden">
...
$test =  $is_demo ? 'test-' : '';
$response =  '<script id="kashier-iFrame" src="https://checkout.kashier.io/kashier-checkout.js" data-amount="'.$amount.'" data-hash="'.$hash.'" data-currency="'.$currency.'" data-orderId="'.$paymentCode.'" data-merchantId="'.$merchant_id.'" data-merchantRedirect="'.urlencode(site_url('payment/kashier')).'" data-serverWebhook="'.urlencode(site_url('payment/kashier')).'"  data-store="Wixout" data-type="external" data-mode="'.$data_mode.'"  data-display="ar"><\/script>';
...
$redirect_url = $checkout_session->url;
    ...
    elseif ($method_id == 32):
        ...
        $paid_amount = $amount * 100;*/
             ...
             "code"=> $paymentCode,
  "description"=> "Add funds",
  "cancel_uri"=>site_url('addfunds'),
  "confirmation_uri"=> site_url(''),
  "callback_uri"=> site_url('payment/airtm'),
  "amount"=> 15,
  "items"=> [
    [
      "description"=> "Add funds",
      ...
      "quantity"=> 1
        ...
        $url = ($is_demo ? 'https://payments.static-stg.tests.airtm.org':'payments.air-pay.io').'/purchases';
        $data =  json_encode($data);
        
        $token = base64_encode($app_key.':'.$secret_key);
...
require_once('lib/mollie/examples/functions.php');       
