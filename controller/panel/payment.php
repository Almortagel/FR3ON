...
$request_body = json_decode($request_body,true);
    ...
    header("Location: ".site_url());
     ...
     elseif ($method_name == 'airtm'): 
          
          file_put_contents('kk',$_REQUEST);
            $order_id = $_REQUEST['oid'];

            $app_key = $extras['app_key'];
            $secret_key = $extras['secret_key'];
            $is_demo = $extras['is_demo'];
           
           
            $url = ($is_demo ? 'https://payments.static-stg.tests.airtm.org':'payments.air-pay.io').'/operations';
        $data =  json_encode($data);
